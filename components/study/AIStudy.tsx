...
export function AIStudy() {
  ...
  async function loadNotes(subject: Subject) {
    ...
    if (json?.ok && Array.isArray(json.notes)) {
      ...
      setSubjects((current) =>
        current.map((item) => (item.id === nextSubject.id ? nextSubject : item)),
      );
    ...
    try {
      const response = await fetch("/api/ai/notes", {
        ...
        body: JSON.stringify({
          ...
          text: noteText.trim() || noteTitle.trim(),
          provider,
        }),
      });
      const json = await response.json();
  ...
  async function loadWhiteboards(subjectId: string) {
    const response = await fetch(`/api/ai/whiteboards?subjectId=${subjectId}`, {
      cache: "no-store",
    });
    const json = await response.json();
  ...
  return (
    <div
      className={
        view === "whiteboard"
          ? "w-full h-screen flex flex-col p-2 sm:p-4"
          : "max-w-6xl mx-auto px-4 sm:px-6 py-6 pb-20 md:pb-6"
      }
    >
      {view === "whiteboard" && (
        <>
          {/* In whiteboard mode the back button is the main way to leave the canvas without losing context. */}
          <button onClick={handleBack} className="p-2 hover:bg-muted rounded-lg w-fit">
      ...
      {view !== "whiteboard" && (
        <div>
          <div className="mb-6">
            ...
            {status && (
              <p
                className={`text-sm mt-2 ${status.includes("saved") || status.includes("created") ? "text-green-600" : "text-red-600"}`}
              >
                {status}
              </p>
            )}
                    ...
                    >
                      ...
                      <div className="grid gap-2">
                        {question.options.map((option) => (
                          <div
                            key={option}
                            className={`p-3 border rounded-lg ${option === question.correctAnswer ? "border-green-400 bg-green-50 dark:bg-green-900/20 dark:border-green-700" : "border-border"}`}
                          >
                            {option}
                          </div>
                        ))}
