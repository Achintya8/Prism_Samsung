    ...
    <nav className="w-full py-5 grid grid-cols-3 items-center">
      ...
      <div>
        <Link href={session ? "/dashboard" : "/"} className="flex items-center gap-2">
          ...
          <span className="text-2xl font-bold tracking-tight">ProductivityHub</span>
        </Link>
      </div>
      ...
      <div className="flex items-center justify-center gap-1">
        {session && (
          <>
            ...
            <Link href="/discussion" className={buttonVariants({ variant: "ghost" })}>
              <MessageSquare className="size-4" />{" "}
              {/* You'll need to import MessageSquare from lucide-react */}
              Discussion
            </Link>
            <Link
              href="/activities"
